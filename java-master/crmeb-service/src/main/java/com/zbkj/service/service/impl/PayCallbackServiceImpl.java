    ...
    public String aliPayCallback(HttpServletRequest request) {
        ...
        try {
            ...
            if (StrUtil.isNotBlank(refundFee)) {// 订单退款
                ...
                RefundOrder refundOrder = refundOrderService.getByRefundOrderNo(outRequestNo);
                ...
                boolean update = refundOrderService.updateById(refundOrder);
                if (update) {
                    // 退款task
...
//                    redisUtil.lPush(TaskConstants.ORDER_TASK_PAY_SUCCESS_AFTER, order.getOrderNo());
    ...
    public String weChatRefund(String xmlInfo) {
        ...
        RefundOrder refundOrder = refundOrderService.getByRefundOrderNo(outRefundNo);
        if (ObjectUtil.isNull(refundOrder)) {
