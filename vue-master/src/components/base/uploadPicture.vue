<template>
  <div>
    <el-row :gutter="30">
      <el-col v-bind="grid" v-hasPermi="['merchant:attachment:list']">
        <div class="Nav">
          <div class="input">
            <el-input
                    ...
                    <span class="el-ic">
                      <el-dropdown>
                        ...
                        <el-dropdown-menu slot="dropdown">
                          <el-dropdown-item @click.native="onAdd(data.id)" v-if="checkPermi(['admin:category:save'])"
                            >添加分类</el-dropdown-item
                          >
                          <el-dropdown-item
                            v-if="node.label !== '全部图片' && checkPermi(['admin:category:update'])"
                            @click.native="onEdit(data.id)"
                            >编辑分类</el-dropdown-item
                          >
                          <el-dropdown-item
                            v-if="node.label !== '全部图片' && checkPermi(['admin:category:delete'])"
        ...
        <div class="conter mb15 relative">
          <div class="bnt">
            ...
            <div class="acea-row" v-if="pictureType">
              <div v-if="typeDate === 'pic'">
                <el-button v-hasPermi="['merchant:upload:image']" class="mr10" type="primary" @click="handleChangeImage"
                  >上传
                </el-button>
              </div>
              <el-upload
                v-hasPermi="['merchant:upload:file']"
                v-else
                class="upload-demo"
                action
                :http-request="handleUploadForm"
          ...
          <div class="pictrueList acea-row" v-loading="loadingPic">
            <div v-show="isShowPic" class="imagesNo">
              ...
              <span class="imagesNo_sp">{{ typeDate.pic ? '图片库为空' : '视频库为空' }}</span>
            </div>
    ...
    >
      <el-form ref="editPram" :model="editPram" label-width="100px" v-loading="loading">
        ...
        <el-form-item v-hasPermi="['merchant:category:save', 'merchant:category:update']">
