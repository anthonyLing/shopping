<template>
  <div class="divBox relative">
    <el-card class="box-card" v-if="checkPermi(['merchant:refund:order:page:list'])">
      <div class="clearfix">
        <div class="container">
    ...
    <el-card class="box-card">
      ...
      >
        ...
        <el-table-column label="操作" min-width="150" fixed="right" align="center" :render-header="renderHeader">
          <template slot-scope="scope">
            <el-button type="text" size="small" @click.native="onOrderDetails(scope.row)">订单详情</el-button>
            <el-button
              type="text"
              size="small"
...
export default {
  ...
  data() {
    return {
      ...
      checkAll: false,
      checkedCities: ['退款单号', '订单号', '用户信息', '退款金额', '退款状态', '创建时间'],
      columnData: ['退款单号', '订单号', '用户信息', '退款金额', '退款状态', '创建时间'],
  ...
  methods: {
    ...
    onOrderRefuse(row) {
      this.$modalPrompt('textarea', '拒绝退款原因').then((V) => {
        orderRefuseApi({ reason: V, refundOrderNo: row.refundOrderNo }).then(() => {
    ...
    onOrderRefund(row) {
      this.$modalSure('您确定要进行退款吗？').then(() => {
        orderRefundApi({ refundOrderNo: row.refundOrderNo }).then((data) => {
          this.$message.success('操作成功');
          this.tableFrom.page = 1;
          this.getList();
        });
    ...
    // 备注
    onOrderMark(row) {
      this.$modalPrompt('textarea', '退款单备注', row.merRemark).then((V) => {
