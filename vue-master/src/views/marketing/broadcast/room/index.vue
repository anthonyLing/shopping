        ...
        <div class="container">
          <el-form size="small" label-width="120px" inline>
            <el-form-item label="状态：" style="display: block">
              <el-radio-group v-model="reviewStatus" type="button" @change="getList(1)">
                <el-radio-button label>全部</el-radio-button>
                <el-radio-button label="0">平台待审核</el-radio-button>
                <el-radio-button label="1">平台审核失败</el-radio-button>
                <el-radio-button label="3">微信审核成功</el-radio-button>
                <el-radio-button label="2">微信审核失败</el-radio-button>
              </el-radio-group>
            </el-form-item>
      ...
      <el-table v-loading="listLoading" :data="tableData.data" style="width: 100%" size="small" highlight-current-row>
        ...
        <el-table-column label="审核状态" min-width="110">
          <template slot-scope="scope">
            <el-tooltip
              ...
              placement="top"
            >
              <el-tag type="danger" v-if="scope.row.reviewStatus === 1">平台审核失败</el-tag>
              <el-tag type="danger" v-if="scope.row.reviewStatus === 2">微信审核失败</el-tag>
            </el-tooltip>
            <div v-else>
              <el-tag type="info" v-if="scope.row.reviewStatus === 0">待审核</el-tag>
              <el-tag v-if="scope.row.reviewStatus === 3">微信审核成功</el-tag>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" min-width="150" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button
              v-hasPermi="['merchant:mp:live:room:addgoods']"
              v-if="Number(scope.row.liveStatus) < 103 && scope.row.reviewStatus === 3 && scope.row.closeGoods === 1"
              type="text"
              size="small"
            ...
            >
            <el-button
              v-hasPermi="['merchant:mp:live:room:mangerass', 'merchant:mp:live:room:getass']"
              v-if="scope.row.reviewStatus === 3"
              type="text"
              size="small"
              class="mr10"
              @click="onEditAssistant(scope.row)"
              >编辑助手</el-button
            >
            ...
            <router-link
              v-hasPermi="['merchant:mp:live:room:edit']"
              v-if="scope.row.liveStatus === 102 && scope.row.reviewStatus === 1 && scope.row.liveStatus === ''"
              :to="{ path: `/marketing/broadcast/creatRoom/${scope.row.id}` }"
            >
              <el-button type="text" size="small" class="mr10">编辑</el-button>
            </router-link>
            <el-button
              v-hasPermi="['merchant:mp:live:room:delete']"
              type="text"
              size="small"
