	...
	<view :data-theme="theme">
		...
		<view class="product-con tui-skeleton" :style="{visibility: showSkeleton ? 'hidden' : 'visible'}">
				...
				<view class='navbarH' :style='"height:"+navH+"rpx;"'>
					<view class='navbarCon acea-row' :style="{ paddingRight: navbarRight + 'px' }">
						...
						<view id="home" class="home acea-row row-center-wrapper iconfont icon-xiangzuo h5_back"
							:class="opacity>0.5?'on':''" :style="{ top: homeTopApp + 'px' }" @tap="returns">
						</view>
						<!-- #endif -->
						...
						<!-- #ifdef H5  || APP-PLUS-->
						<view class="right_select" :style="{ top: homeTopApp + 'px' }" @tap="showNav">
			...
			<view class="detail_container">
				<scroll-view :scroll-top="scrollTop" scroll-y='true' scroll-with-animation="true"
					...
					<view id="past0">
						...
						<!-- 秒杀card -->
						<seckill-card v-if="type==='seckill'" :seckillStatus="seckillStatus" :seckillTime="seckillTime"
							:productInfo="productInfo"></seckill-card>
						<!-- 氛围图card -->
						<activity-style v-if="productInfo.activityStyle" :productInfo="productInfo"></activity-style>

						<view class="borderPad" :class="type==='seckill'? 'mtop': ''">
							<view class='wrapper mb20 borRadius14'>
								<view v-if="type =='normal' && !productInfo.activityStyle">
									<view class='share acea-row row-between row-bottom mb10'>
										<view class='x-money flex align-baseline'>
											￥
											<text
												class='num font-44 tui-skeleton-rect'>{{attr.productSelect.price}}</text>
											<view class="flex pl-2 tui-skeleton-rect"
												v-if="attr.productSelect.vipPrice && attr.productSelect.vipPrice > 0">
												<image :src="urlDomain+'crmebimage/presets/vip_badge.png'"
													class="vip_icon tui-skeleton-rect"></image>
												<text
													class='vip_money tui-skeleton-rect'>￥{{attr.productSelect.vipPrice}}</text>
											</view>
										</view>
									...
									<view v-if="couponList.length>0" class="acea-row row-between-wrapper">
										<view class="flex-1 acea-row row-middle">
											<span v-for="(item, index) in couponList"
												class="coupon_label">{{item.minPrice===0?'无门槛减'+item.money:'满'+item.minPrice+'减'+item.money}}</span>
										</view>
										<view class="coupon_more" @click='couponTap'>领券<text
												class="iconfont icon-you"></text> </view>
									</view>
									<view class='introduce tui-skeleton-rect line2 mt30'>{{productInfo.name}}</view>
								</view>
								<view v-if="type !=='normal'" class='share acea-row row-between row-bottom'>
								...
								<view class='label acea-row row-between-wrapper'>
									<view class="tui-skeleton-rect">原价:￥{{attr.productSelect.otPrice || 0}}</view>
								...
								<view class="coupon acea-row row-between-wrapper" v-if="activityH5.length">
									<view class="line1 acea-row">
										...
										<view v-for='(item,index) in activityH5' :key='index' @click="goActivity(item)"
											class="activityBox">
											<view v-if="item.type === '1'" class="tui-skeleton-rect"
							...
							<view class='attribute mb20 borRadius14 tui-skeleton-rect'>
								...
								<view v-if="guaranteeList.length>0" class="acea-row row-between-wrapper"
									...
									<view class="line1 tui-skeleton-fillet"> <text
											class="text-666 fw-bold">保&nbsp;&nbsp;&nbsp;障：</text>
					...
					<view class="borderPad">
						<view class="superior borRadius14" if='merchantInfo'>
							<merHome :merchantInfo="merchantInfo" :merid="productInfo.merId" type="home"
								:isShowTypeId="isShowTypeId"></merHome>
			...
			<view class='footer acea-row row-between-wrapper'>
				<navigator v-if="type !== 'normal'" :url="`/pages/merchant/home/index?id=${productInfo.merId}`"
					hover-class="none">
				...
				<block v-if="type === 'normal'">
					...
					<view class="bnt acea-row" v-if="attr.productSelect.stock <= 0">
						<form report-submit="true"><button class="longBnts bg-color-hui" form-type="submit">已售罄</button>
						</form>
					</view>
				...
				<view v-if="attr.productSelect.stock <= 0 && type !== 'normal'" class="bnt bntVideo acea-row">
					<form report-submit="true"><button class="longBnts bg-color-hui" form-type="submit">已售罄</button>
					</form>
				</view>
				<view v-if="seckillStatus !==1 && type === 'seckill'&&attr.productSelect.stock > 0 "
					class="bnt acea-row">
					<form @submit="goDetail(masterProductId)" report-submit="true"><button class="joinCart bnts"
							form-type="submit">直接购买</button></form>
					<form report-submit="true"><button class="bg-color-hui bnts"
							form-type="submit">{{seckillStatus===0?'活动已结束':'活动未开始'}}</button>
					</form>
				</view>
				<view
					v-if="attr.productSelect.stock > 0 && (type === 'video'|| (type === 'seckill' && seckillStatus ===1))"
					class="bnt bntVideo acea-row">
					<form @submit="goBuy" report-submit="true"><button class="buy bg-color longBnts"
							form-type="submit">立即购买</button>
					</form>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			//typeNum商品类型:0-普通normal，1-秒杀seckill，2-砍价，3-拼团，4-视频号video
			if (options.typeNum) {
				let type = filters.orderTypeValFilter(options.typeNum);
				this.type = type;
			} else {
		...
		onShareAppMessage: function(res) {
			...
			return {
				...
				path: `/pages/goods/goods_details/index?id=${that.id}&spread=${that.uid}&type=${this.type}`
			}
		},
		// #endif
		methods: {
			...
			getTokenIsExist() {
				tokenIsExistApi().then(res => {
					...
					silenceBindingSpread(this.globalData, this.tokenIsExist)
				})
			},
			getMerCustomer(id) {
			...
			appShare(scene) {
				...
				uni.share({
					...
					scene: scene,
					type: 0,
					href: `${HTTP_H5_URL}${curRoute}&spread=${that.uid}&type=${this.type}`,
					title: that.productInfo.name,
			...
			getGoodsDetails: function() {
				...
				getProductDetail(that.id, that.type).then(res => {
					...
					//if (this.merchantInfo.proList.length>0) navList.splice(2, 0, '推荐')
			...
			getSeckillInfo(data) {
				...
				if (!this.options.datatime && this.type === 'seckill') {
			...
			getProductReplyList: function() {
				let id = Number(this.masterProductId) > 0 ? this.masterProductId : this.id
			...
			getProductReplyCount: function() {
				let that = this;
				let id = Number(that.masterProductId) > 0 ? that.masterProductId : that.id
			...
			setCollect: Debounce(function() {
				...
				if (this.isLogin === false) {
					...
					let id = Number(this.masterProductId) > 0 ? this.masterProductId : this.productInfo.id
					if (this.userCollect) {
						collectCancel({
							ids: id
						}).then(res => {
			...
			getQrcode() {
				let that = this;
				let data = {
					scene: 'id=' + this.id + '&spread=' + that.uid + '&type=' + that.type,
			...
			// 生成二维码；
			make(uid) {
				let href = location.href.split('?')[0] + "?id=" + this.id + "&spread=" + this.uid + "&type=" + this.type;
				uQRCode.make({
					canvasId: 'qrcode',
					text: href,
